...
from .feature import CombinedWindow
...
class PCA(BaseDetector):
    ...
    def __init__(self, slidingWindow=100, sub = True, n_components=0.25, n_selected_components=None,
    ...
    def fit(self, X, y=None):
        ...
        self : object
            Fitted estimator.
        """
        data_imputed = X["data_imputed"]
        true_data = X["data"]
        n_samples, n_features = true_data.shape
        # Converting time series data into matrix format
        X = CombinedWindow(window = self.slidingWindow).convert(data_imputed, true_data)
        if self.normalize: 
            if n_features == 1:
    ...
    def decision_function(self, X):
        ...
        # Converting time series data into matrix format
        X = CombinedWindow(window = self.slidingWindow).convert(X, true_data=X["data"])
        if self.normalize: 
            if n_features == 1:
                X = zscore(X, axis=0, ddof=0)
            else: 
