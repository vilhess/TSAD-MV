...
class MTAD_GAT(nn.Module):
    ...
    def __init__(
        ...
        self.forecasting_model = Forecasting_Model(gru_hid_dim, forecast_hid_dim, n_features, forecast_n_layers, dropout)
        self.recon_model = ReconstructionModel(window_size, gru_hid_dim, recon_hid_dim, n_features, recon_n_layers, dropout)
...
class MTAD_Detector(BaseDetector):
    def __init__(self,
    ...
    def fit(self, data):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for idx, (x, _) in loop:                
                ...
                preds, recons = self.model(context)
                forecast_loss = torch.sqrt(forecast_criterion(preds, target))
                recon_loss = torch.sqrt(recon_criterion(recons, context))
            ...
            if len(valid_loader) > 0:
                ...
                with torch.no_grad():
                    for idx, (x, _) in loop:      
                        ...
                        preds_val.append(forecast_loss.cpu().item())
                        recons_loss_val.append(recon_loss.cpu().item())
                        loop.set_description(f"Validation Epoch [{epoch}/{self.epochs}]")
                        loop.set_postfix(preds_val=sum(preds_val) / (idx + 1), recons_loss_val=sum(recons_loss_val) / (idx + 1))

            forecast_loss = np.array(preds_val)
            recon_loss = np.array(recons_loss_val)

            forecast_loss = np.sqrt(((forecast_loss)**2).mean())
            recon_loss = np.sqrt(((recon_loss)**2).mean())
    ...
    def decision_function(self, all_datas):
        data = self.scaler.transform(all_datas["data_imputed"])
        true_data = self.scaler.transform(all_datas["data"]) # last timestep per window never contains missing value 
        mask = all_datas["mask"]
    
        test_loader = DataLoader(
            dataset=ReconstructCombinedDataset(data, true_data, window_size=self.win_size),
            batch_size=self.batch_size,
            shuffle=False,
        )
        ...
        with torch.no_grad():
            for idx, (x, _) in loop:
                ...
                context = x[:, :-1, :]
                target = x[:, -1, :]
                preds, _ = self.model(context)
                recon_x = torch.cat([context[:, 1:, :], target.unsqueeze(1)], dim=1)
                _ , recons = self.model(recon_x)
                loss_preds = torch.sqrt((preds - target)**2).squeeze(1)
                loss_recons = torch.sqrt((recons[:, -1, :] - context[:, -1, :])**2).mean(dim=1)
                loss = loss_preds + self.gamma*loss_recons
